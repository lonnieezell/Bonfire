<div class="box select admin-box">
    <?php echo form_open(SITE_AREA . "/reports/activities/{$vars['which']}", 'class="form-horizontal constrained"'); ?>
        <fieldset>
            <legend><?php echo lang('activity_filter_head'); ?></legend>
            <?php
        ...
        <fieldset class="form-actions">
            <?php
            echo form_submit('filter', lang('activity_filter'), 'class="btn btn-primary"');
            if ($vars['which'] == 'activity_own' && has_permission('Activities.Own.Delete')) :
            ?>
            <button type="submit" name="delete" class="btn btn-danger" id="delete-activity_own"><span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('activity_own_delete'); ?></button>
            <?php elseif ($vars['which'] == 'activity_user' && has_permission('Activities.User.Delete')) : ?>
            <button type="submit" name="delete" class="btn btn-danger" id="delete-activity_user"><span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('activity_user_delete'); ?></button>
            <?php elseif ($vars['which'] == 'activity_module' && has_permission('Activities.Module.Delete')) : ?>
            <button type="submit" name="delete" class="btn btn-danger" id="delete-activity_module"><span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('activity_module_delete'); ?></button>
            <?php elseif ($vars['which'] == 'activity_date' && has_permission('Activities.Date.Delete')) : ?>
            <button type="submit" name="delete" class="btn btn-danger" id="delete-activity_date"><span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('activity_date_delete'); ?></button>
            <?php endif; ?>
        </fieldset>
    <?php echo form_close(); ?>
</div>
