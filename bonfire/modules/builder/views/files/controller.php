...
if ($db_required != '') {
    $mb_index .= "
		...
		if (\$this->input->get_post('search') != \"\") {
		    ...
		    //Real FULLTEXT search (to enable, read comments in the model class)
		    //\$total = count(\$this->" . $module_name_lower . "_model->ft_search(\$this->input->get_post('search')));
		    //\$records = \$this->" . $module_name_lower . "_model->ft_search(\$this->input->get_post('search'), \$limit, \$offset);

		    //Simple \"LIKE based\" search
		    \$total = count(\$this->" . $module_name_lower . "_model->search(\$this->input->get_post('search')));
		    \$records = \$this->" . $module_name_lower . "_model->search(\$this->input->get_post('search'), \$limit, \$offset);
		    \$this->pager['base_url'] = current_url() . '?search=' . \$this->input->get_post('search');
		} else {
		    //simple records list
...
for ($counter = 1; $field_total >= $counter; $counter++) {
    ...
    $view_field_type = set_value("view_field_type$counter");
    $view_datepicker = '';
	...
	} elseif (($db_field_type == 'TEXT' || $db_field_type == 'MEDIUMTEXT' || $db_field_type == 'LONGTEXT') && ($view_field_type == 'wysiwyg') && $textarea_included === FALSE && !empty($textarea_editor)
	) {
	    // if a date field hasn't been included already then add in the jquery ui files
	    if ($textarea_editor == 'ckeditor') {
		$extras .= '
			Assets::add_js(\'../editors/ckeditor/ckeditor.js\');
			Assets::add_js(\'../editors/ckeditor/ckconfig.js\');';
            } elseif ($textarea_editor == 'elrte') {
                $extras .= '
		/**
		 * ElRte editor with integrated ElFinder file manager.
		 * Before use, change the connector configs in connector controller
		 *
		 * @author: Lorenzo Sanzari (ulisse73@quipo.it)
		 */

		Assets::add_css(\'../editors/elrte/css/smoothness/jquery-ui-1.9.1.custom.min.css\');
		//ElRte CSS
		Assets::add_css(\'../editors/elrte/css/elrte.min.css\');
		Assets::add_css(\'../editors/elrte/css/elrte-inner.css\');
		//ElFinder CSS
		Assets::add_css(\'../editors/elrte/elfinder/css/elfinder.min.css\');
		Assets::add_css(\'../editors/elrte/elfinder/css/theme.css\');
		//ElRte JS
		Assets::add_js(\'../editors/elrte/js/jquery-ui-1.9.1.custom.min.js\');
		Assets::add_js(\'../editors/elrte/js/jquery.json-2.4.js\');
		Assets::add_js(\'../editors/elrte/js/elrte.full.js\');
		Assets::add_js(\'../editors/elrte/js/i18n/elrte.it.js\');
		//ElFinder JS
		Assets::add_js(\'../editors/elrte/elfinder/js/elfinder.min.js\');
		Assets::add_js(\'../editors/elrte/elfinder/js/commands/all.js\');
		Assets::add_js(\'../editors/elrte/elfinder/js/i18n/elfinder.en.js\');

';

            } elseif ($textarea_editor == 'xinha') {
                $extras .= '
			Assets::add_js(\'../editors/xinha/xinha_conf.js\');
			Assets::add_js(\'../editors/xinha/XinhaCore.js\');';
            } elseif ($textarea_editor == 'markitup') {
                $extras .= '
			Assets::add_css(\'../editors/markitup/skins/markitup/style.css\');
			Assets::add_css(\'../editors/markitup/sets/default/style.css\');

			Assets::add_js(\'../editors/markitup/jquery.markitup.js\');
			Assets::add_js(\'../editors/markitup/sets/default/set.js\');';
            } elseif ($textarea_editor == 'tinymce') {
                $extras .= '
			Assets::add_js(\'../editors/tiny_mce/tiny_mce.js\');
			Assets::add_js(\'../editors/tiny_mce/tiny_mce_init.js\');';
            }
