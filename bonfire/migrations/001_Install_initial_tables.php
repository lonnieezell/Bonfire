...
class Migration_Install_initial_tables extends Migration
{
	...
	public function up()
	{
		...
		foreach ($this->permissions_data as $key => $permissions)
		{
			$current_permission = array();
			foreach ($permissions as $key => $val)
			{
				$current_permission[$this->db->database . '.' . $this->permissions_table . '.' . $key] = $val;
			}
			$permission_data[] = $current_permission;
		}

		$this->db->insert_batch($this->permissions_table, $permission_data);
